...
/* gsfuncs.c                            */
extern  char    *scr_multi_funcs( char *in, char *pstart, char **result, int ressize );
...
#define pick( name, length, mand_parms, opt_parms, routine ) \
    extern condcode routine( parm parms[MAX_FUN_PARMS], int parm_count, char **result, int ressize );
#include "gsfuncs.h"
#undef pick
