        ...
        case el_text :
            if( cur_el->element.text.first == NULL ) {          // empty heading
                ...
                for( cur_line = cur_el->element.text.first; cur_line != NULL;
                    ...
                    if( ProcFlags.ps_device ) {     // PS only
                        if( shade_done ) {          // prior line was shade
                            if( cur_line->first->font == PS_SHADE_FONT ) {  // current line is shade
                                cur_el->element.text.overprint = false;     // cancel overprint
                            }
                            shade_done = false;                             // clear flag
                        } else {                    // prior line was not shade
                            if( (cur_line->first != NULL)
                                    && (cur_line->first->font == PS_SHADE_FONT) ) {  // current line is shade
                                shade_done = true;                          // set flag
                            }
                        }
                    }
...
void do_page_out( void )
{
    ...
    if( apage && WgmlFlags.lastpass ) {   // don't do before first page
